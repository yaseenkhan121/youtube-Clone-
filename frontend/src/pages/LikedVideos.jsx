...
import { ThumbsUp, Trash2, Heart, HeartOff, RefreshCcw } from 'lucide-react';
...
const LikedVideos = () => {
    ...
    const fetchLiked = async () => {
        try {
            ...
            setError(err.response?.status === 401 ? 'Login to see liked videos' : 'Failed to load liked videos');
        } finally {
            setLoading(false);
        }
    };
    ...
    useEffect(() => {
        if (isLoggedIn) fetchLiked();
        else setLoading(false);
    }, [isLoggedIn]);

    const handleUnlike = async (e, video) => {
        e.stopPropagation();
        try {
            const res = await videoService.toggleLike({
    ...
    if (!isLoggedIn) return (
        <div className="flex flex-col items-center justify-center min-h-[70vh] p-20 text-center gap-6">
            ...
            <div className="space-y-2">
                <h2 className="text-3xl font-black text-white tracking-tight">Your favorite moments</h2>
                <p className="text-text-secondary font-medium">Liked videos are only accessible when you're signed in.</p>
            </div>
            <button 
        ...
        <div className="max-w-[1100px] mx-auto p-4 md:p-8 pb-32">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-12">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-black text-white tracking-tighter">Liked Videos</h1>
                        <p className="text-text-secondary text-sm font-medium">{videos.length} items in your personal collection</p>
                    </div>
                </div>
                <button 
                    onClick={fetchLiked}
            ...
            {videos.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-32 text-center bg-card-bg/20 rounded-[3rem] border border-dashed border-border-color">
                    <HeartOff size={64} className="text-neutral-800 mb-6" />
                    <h3 className="text-xl font-black text-white mb-2">Feeling empty?</h3>
                    <p className="text-text-secondary font-medium">Show some love to videos you enjoy to see them here.</p>
                    <button onClick={() => navigate('/')} className="mt-8 text-primary font-black hover:underline underline-offset-4">Explore Content</button>
                </div>
            ) : (
